...
for CLIENT in ${CLIENT_LIST[@]}; do 
  ...
  iptables -I FORWARD -i ${WIREGUARD_NAME} -s ${VPN_CLIENT_IP} -d ${ALL_MACHINE_SUBNET} -j DROP
  iptables -I FORWARD -i ${WIREGUARD_NAME} -s ${TEAM_SERVER_IP} -d ${VPN_CLIENT_IP} -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
  iptables -I FORWARD -i ${WIREGUARD_NAME} -s ${VPN_CLIENT_IP} -d ${TEAM_SERVER_IP} -j ACCEPT
done
