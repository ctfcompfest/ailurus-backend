...
public_challenge_blueprint = Blueprint("public_challenge", __name__, "/challenges")
public_challenge_blueprint.before_request(validteam_only)
...
def get_detail_challenges(challenge_id):
    ...
    chall: Challenge = Challenge.query.filter_by(id=id).first()
    chall_data = {
        "id": chall.id,
        "title": chall.title,
